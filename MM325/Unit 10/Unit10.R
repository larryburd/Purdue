...
# Left sided chi-square test critical value
qchisq(a, df)
...
dfStats <- data.frame(DegreesOfFreedom = df, SampleVariance = smplVar,
                      SampleStandardDev = smplStDev, PopulationVariance = popVar)
dfStats
...
# Create a matrix to hold the data
data <- matrix(c(18, 64, 20, 23, 30, 49, 14, 18, 45, 61, 20, 22), nrow = 3, byrow = TRUE)
